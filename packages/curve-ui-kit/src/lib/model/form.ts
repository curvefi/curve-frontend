...
/**
 * Wrapper to call form.watchValues() and avoid lint errors about incompatible hooks.
 * The `eslint-disable-next-line react-hooks/incompatible-library` should be needed here, but eslint misses it.
 */
export const watchForm = <TFieldValues extends FieldValues>(form: UseFormReturn<TFieldValues>) => form.watchValues()

/**
 * Wrapper to call form.watchValue() and avoid lint errors about incompatible hooks.
 * The `eslint-disable-next-line react-hooks/incompatible-library` should be needed here, but eslint misses it.
 */
export const watchField = <TFieldValues extends FieldValues, TFieldName extends FieldPath<TFieldValues>>(
  form: UseFormReturn<TFieldValues>,
  name: TFieldName,
): FieldPathValue<TFieldValues, TFieldName> => form.watchValue(name)
