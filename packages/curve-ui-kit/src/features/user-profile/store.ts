...
type State = {
  ...
  locale: Locale['value']
  /**
   * Key is usually a chainIdPoolId from getChainPoolIdActiveKey,
   * or just 'router' if it's global. Perhaps we should rename it to 'global',
   * but haven't done so yet for compatibility reasons.
   */
  maxSlippage: { router: string } & Partial<Record<string, string>>
  ...
  setMaxSlippage: (maxSlippage: string | null, key?: string) => {
    ...
    set((state) => ({
      ...
      maxSlippage: key
        ...
        : Object.keys(state.maxSlippage).reduce((acc, k) => ({ ...acc, [k]: maxSlippage }), { router: maxSlippage }),
    }))
