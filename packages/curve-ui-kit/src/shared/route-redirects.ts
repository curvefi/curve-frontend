 ...
 */
export function getHashRedirectUrl({ pathname: path, search, hash, host }: Location, newNetworkId: string) {
  ...
  if ([app, network].includes('integrations')) {
    ...
    return `/${app === 'integrations' ? 'dex' : app}/${newNetworkId}/integrations/${search}`
  }
  ...
  if (network && routes?.find((r) => r.startsWith(`/${network}`))) {
    ...
    return `/${appName}/${newNetworkId}/${network}/${rest.join('/')}${search}`
  }
  const restPath = rest.filter(Boolean).length ? rest.join('/') : defaultPages[appName]
  return `/${app || 'dex'}/${network || newNetworkId}/${restPath}${search}`
}
