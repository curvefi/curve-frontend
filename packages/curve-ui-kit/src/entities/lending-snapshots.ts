...
export const { useQuery: useLendingSnapshots } = queryFactory({
  ...
  queryFn: async ({ blockchainId, contractAddress, agg, timeOption, limit }: Query): Promise<LendingSnapshot[]> => {
    ...
    const response = await getSnapshots(blockchainId, contractAddress, apiParams as Parameters<typeof getSnapshots>[2])
