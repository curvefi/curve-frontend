...
export const parseTopPools = (x: Responses.GetTopPoolsResponse['revenue'][number]): Models.ChainTopPoolRevenue => ({
  name: x.name,
  totalDailyFeesUSD: x.totalDailyFeesUSD,
})
...
export const parseCushion = (x: Responses.GetCushionsResponse['data'][number]): Models.Cushion => ({
  pool: x.pool,
  name: x.name,
...
): Models.Distribution => ({ timestamp: x.timestamp, feesUsd: x.fees_usd })
...
export const parseFeesCollected = (x: Responses.GetFeesCollectedResponse['data'][number]): Models.FeesCollected => ({
  coin: {
    lpToken: x.coin.lp_token,
    symbol: x.coin.symbol,
    address: x.coin.address,
...
export const parseFeesStaged = (x: Responses.GetFeesStagedResponse['data'][number]): Models.FeesStaged => ({
  coin: {
    lpToken: x.coin.lp_token,
    symbol: x.coin.symbol,
    address: x.coin.address,
