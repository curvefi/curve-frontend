...
describe('CreateLoanForm Component Tests', () => {
  ...
  function BorrowTabTest({ type, onCreated }: BorrowTabTestProps) {
    ...
    const market = useMemo(
      () =>
        isHydrated &&
        { [LlamaMarketType.Mint]: llamaApi?.getMintMarket, [LlamaMarketType.Lend]: llamaApi?.getLendMarket }[type]?.(
          id,
        ),
      [isHydrated, llamaApi?.getLendMarket, llamaApi?.getMintMarket, type],
    )
    return market ? (
