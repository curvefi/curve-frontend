...
const TableRowContent = ({
  ...
  const content: { borrow: Content[][], supply: Content[][] } = {
    [FilterType.borrow]: [
      ...
      [
        { tableKey: TITLE.rateBorrow, content: <CellRate {...cellProps} type='borrow' /> },
      ],
      [
    ...
    [FilterType.supply]: [
      ...
      [
        { tableKey: TITLE.totalApr, content: <CellRewards {...cellProps} type='crv-other' /> }
      ]
    ]
  }
                    ...
                    <React.Fragment key={detailsKey}>
                      <ListInfoItems>
                        {details.map(({ tableKey, content, show }, idx) => {
                          ...
                          return (
                            <ListInfoItem key={`info-${idx}`} title={TITLE_MAPPER[tableKey]?.name ?? ''}>
                              {content}
                            </ListInfoItem>
                          )
                        })}
