...
Array.prototype.dropWhile = function <T>(this: T[], predicate: (value: T, index: number, array: T[]) => boolean) {
  const index = this.findIndex((value, index, array) => !predicate(value, index, array))
...
Array.prototype.uniqWith = function <T>(this: T[], comparator: (a: T, b: T) => boolean) {
  return this.filter((element, index) => this.findIndex((step) => comparator(element, step)) === index)
}
...
Array.prototype.groupBy = function <T, K extends PropertyKey>(this: T[], keyFn: (item: T) => K) {
...
Array.prototype.meanBy = function <T>(this: T[], iteratee: (item: T) => number) {
...
Array.prototype.maxBy = function <T>(this: T[], iteratee: (item: T) => number) {
  return this.reduce((max, current) => (iteratee(current) > iteratee(max) ? current : max))
}
