...
const createLoanSwap = (set: SetState<State>, get: GetState<State>) => ({
  [sliceKey]: {
    ...
    setFormValues: async (chainId: ChainId, llamma: Llamma, formValues: FormValues, maxSlippage: string) => {
      ...
      const loadingDetailInfo = lodash.cloneDeep(
      ...
      get()[sliceKey].setStateByKeys({ activeKey, formValues: lodash.cloneDeep(cFormValues) })
      ...
      get()[sliceKey].setStateByKey('formValues', lodash.cloneDeep(cFormValues))
