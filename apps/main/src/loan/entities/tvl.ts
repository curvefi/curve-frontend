import { getCrvUsdTvl, type CrvUsdTvl } from '@curvefi/prices-api/crvusd'
...
export const { useQuery: useCrvUsdTvl } = queryFactory({
  queryKey: ({ blockchainId }: ChainNameParams) => ['crvusdMarketTvl', { blockchainId: blockchainId }, 'v1'] as const,
  queryFn: ({ blockchainId }: ChainNameQuery): Promise<CrvUsdTvl> => getCrvUsdTvl(blockchainId),
