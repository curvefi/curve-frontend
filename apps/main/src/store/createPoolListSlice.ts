...
const createPoolListSlice = (set: SetState<State>, get: GetState<State>) => ({
  [sliceKey]: {
    ...
    filterBySearchText: (searchTerm: string, poolDatas: (PoolDataCache | PoolData)[]) => {
      ...
      const { addressesResult, tokensResult } = searchByText(
        searchTerm,
        poolDatas,
        [SEARCH_TERM['pool.name'], SEARCH_TERM['pool.wrappedCoins'], SEARCH_TERM['pool.underlyingCoin']],
        [
          SEARCH_TERM['pool.wrappedCoinAddresses'],
          SEARCH_TERM['pool.underlyingCoinAddresses'],
          SEARCH_TERM['pool.address'],
          SEARCH_TERM['pool.gauge.address'],
          SEARCH_TERM['pool.lpToken'],
        ]
      )
    ...
    setFormValues: (
      ...
      sliceState.setStateByKeys({ activeKey, formValues, searchedByAddresses: {}, searchedByTokens: {} })
