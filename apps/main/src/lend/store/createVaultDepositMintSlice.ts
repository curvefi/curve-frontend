...
const createVaultMint = (
  ...
  [sliceKey]: {
    ...
    setFormValues: async (rChainId, formType, api, market, partialFormValues) => {
      ...
      // api calls
      get()[sliceKey].fetchDetails(activeKey, formType, market).catch(errorFallback)

      if (signerAddress) {
        await get()[sliceKey].fetchMax(rChainId, formType, market)
        get()[sliceKey].fetchEstGasApproval(activeKey, formType, api, market).catch(errorFallback)
      }
    },
