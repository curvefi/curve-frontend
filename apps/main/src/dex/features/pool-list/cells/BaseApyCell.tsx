...
const BaseApyTooltipContent = ({
  baseDailyApr,
  baseWeeklyApr,
  weekly,
}: Pick<PoolRow, 'baseDailyApr' | 'baseWeeklyApr'> & { weekly: boolean }) => {
  const baseDailyApy = aprToPoolApy(baseDailyApr)
  const baseWeeklyApy = aprToPoolApy(baseWeeklyApr)
  const hasNegativeApy = [baseDailyApy, baseWeeklyApy].some(apy => apy != null && apy < 0)
      ...
      <TooltipWrapper>
        ...
        <TooltipItems secondary>
          ...
          <TooltipItem variant="independent" title={t`Weekly`}>
            {formatNumber(baseWeeklyApy, 'percent.rate')}
          </TooltipItem>
        </TooltipItems>
        {hasNegativeApy && (
  ...
  return (
    ...
    >
      <Tooltip
        ...
        title={weekly ? t`Weekly Base APY` : t`Base APY`}
        body={
          <BaseApyTooltipContent baseDailyApr={pool.baseDailyApr} baseWeeklyApr={pool.baseWeeklyApr} weekly={weekly} />
        }
        placement="top"
      >
        <Box
