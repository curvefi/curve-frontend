...
import type { RefuelFormValues, RefuelTokens } from '../types'
...
export const useRefuelMutation = ({ chainId, poolAddress, tokens, userAddress, ...props }: RefuelMutationOptions) => {
    ...
    mutationFn: async (form: RefuelFormValues) => {
      ...
      for (const { amount, token } of [
        ...
        await waitForApproval({
          ...
          onApprove: async () => await approve(config, { chainId, tokenAddress, spenderAddress }),
