...
import { invalideBridgeIsApproved, useBridgeIsApproved } from '../queries/bridge-is-approved'
...
export const useBridgeForm = ({ chainId, networks }: { chainId: number; networks: Record<number, NetworkConfig> }) => {
  // Form
  ...
  useEffect(() => {
    form.setValue('walletBalance', walletBalance.balance, setValueOptions)
  ...
  useEffect(() => {
    form.setValue('min', capacity?.min, setValueOptions)
    form.setValue('max', capacity?.max, setValueOptions)
  ...
  const {
    ...
    chainId,
    onApproved: async () => {
      await invalideBridgeIsApproved(params)
    },
  })
  ...
  useEffect(() => {
    ...
    form.setValue('fromChainId', network?.chainId ?? supportedNetworks[0]?.chainId, setValueOptions)
