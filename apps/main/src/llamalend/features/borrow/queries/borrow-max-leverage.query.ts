...
import { type PreciseNumber, toPrecise, Zero } from '@ui-kit/utils'
...
export const { useQuery: useMaxLeverage } = queryFactory({
  ...
  queryFn: async ({ poolId, range }: BorrowMaxLeverageQuery): Promise<PreciseNumber> => {
    const market = getLlamaMarket(poolId)
    return hasLeverage(market)
      ? market instanceof MintMarketTemplate && market.leverageV2.hasLeverage()
        ? toPrecise(await market.leverageV2.maxLeverage(range))
        : toPrecise(await market.leverage.maxLeverage(range))
      : Zero
  },
  staleTime: '1m',
