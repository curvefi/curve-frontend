...
import { generateOracleMarkLines, generateRangeMarkLines, type MarkLine } from './markLines'
import type { ChartDataPoint, BandsChartPalette, BandsRangeOverlay, DerivedChartData } from './types'

type RectSeriesData = [number, number, number, number, number, number, number][]
type RangeSeriesData = [number, number, number, number][]

const getOverlayPrices = (rangeOverlays: BandsRangeOverlay[]) =>
  rangeOverlays.flatMap(({ lowerPrice, upperPrice }) => [lowerPrice, upperPrice]).filter(Number.isFinite)
...
const getPriceMin = (
  ...
  const min = Math.min(...chartData.map(d => d.p_down), ...rangePrices)
  ...
  const oraclePriceValue = Number(oraclePrice)
  const lowestPrice = Number.isFinite(oraclePriceValue) ? Math.min(min, oraclePriceValue) : min
  // if oraclePrice is outside of range of bands, set min to oraclePrice - bandDelta to make sure it's visible
  return lowestPrice - bandDelta * 2
}
...
const getPriceMax = (
  ...
  const rangePrices = getOverlayPrices(rangeOverlays)
  const max = Math.max(...chartData.map(d => d.p_up), ...rangePrices)
  // bandDelta ensures padding to prevent label clipping if a label is too close to the edge
  const bandDelta = chartData[0].p_down - chartData[0].p_up
  const oraclePriceValue = Number(oraclePrice)
  const highestPrice = Number.isFinite(oraclePriceValue) ? Math.max(max, oraclePriceValue) : max
  // if oraclePrice is outside of range of bands, set max to oraclePrice + bandDelta to make sure it's visible
  return highestPrice + bandDelta * 2
}

const LINE_WIDTH = 0.5
const [ENABLE_OUTLINE, DISABLE_OUTLINE] = [true, false]

// ECharts renders larger z values above smaller ones.
// Keep the full stack here so layer changes are reviewed as one ordered policy.
const CHART_LAYER = {
  currentRangeArea: 0,
  currentRangeLine: 1,
  newRangeArea: 2,
...
const createMarkLineOption = (markLines: MarkLine[], z2: number) =>
  markLines.length
    ? {
        animation: false,
        animationDuration: 0,
  ...
  return {
    ...
    series: (() => {
      ...
      const currentRangeLineSeries = currentRangeOverlays.map((rangeOverlay, index) =>
        createMarkerOnlySeries(
          `Current Liquidation Range Line ${index + 1}`,
          outlineSeriesData,
          CHART_LAYER.currentRangeLine,
          undefined,
          createMarkLineOption(
            generateRangeMarkLines(rangeOverlay, xStart, xEnd, newRangeOverlays),
            CHART_LAYER.currentRangeLine,
          ),
        ),
      )
      ...
      const newRangeLineSeries = newRangeOverlays.map((rangeOverlay, index) =>
        createMarkerOnlySeries(
          ...
          undefined,
          createMarkLineOption(generateRangeMarkLines(rangeOverlay, xStart, xEnd), CHART_LAYER.newRangeLine),
        ),
      )

      const oracleMarkLine = createMarkLineOption(
        generateOracleMarkLines(oraclePrice, xStart, xEnd, palette),
        CHART_LAYER.oracleLine,
      )
      const oracleLineSeries = oracleMarkLine
        ? [
            createMarkerOnlySeries(
              'Oracle Price Line',
              outlineSeriesData,
