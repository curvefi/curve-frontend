...
import {
  invalidateLlamaMarketsOnchainData,
  useLlamaMarketsTableData,
} from '../../queries/market-list/llama-markets-table-data'
...
const useOnReload = ({ address: userAddress, isFetching }: { address?: Address; isFetching: boolean }) => {
  ...
  const onReload = useCallback(() => {
    ...
    void Promise.all([
      ...
      invalidateLlamaMarketsOnchainData(userAddress),
    ])
...
export const LlamaMarketsList = () => {
  ...
  const { data, isError, isLoading, isFetching } = useLlamaMarketsTableData(address)
