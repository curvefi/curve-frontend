...
export async function clientLoader() {
  ...
  // Get preferred color scheme from headers if available
  const preferredScheme = null // React Router doesn't provide headers in clientLoader
  return { networks, preferredScheme }
}

export function Layout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en" style={RootCssProperties}>
      <head>
        <meta charSet="UTF-8" />
...
export default function Root() {
  const { networks, preferredScheme } = useLoaderData<typeof clientLoader>()
