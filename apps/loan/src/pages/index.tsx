...
const App: NextPage = () => {
  const { locale } = getStorageValue('APP_CACHE') ?? {}
  const parsedLocale = parseLocale(locale)

  const SubRoutes = (
    <>
      <Route path=":network" element={<Navigate to="markets" replace />} />
  ...
  return (
    <Routes>
      ...
      <Route path="/" element={<Navigate to={`${parsedLocale.pathnameLocale}/ethereum/markets`} replace />} />
