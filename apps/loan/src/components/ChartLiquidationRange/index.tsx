...
const ChartLiquidationRange = ({ data, healthColorKey, isManage, isDetailView, theme }: Props) => {
        ...
        <ResponsiveContainer width={'99%'} height={chartHeight}>
          <ComposedChart layout="vertical" barGap={-30} data={data} margin={{ left: 5, top: 10 }}>
            <XAxis
              ...
              tick={{ fontSize: 12 }}
              tickFormatter={(tick) => `${formatNumber(tick, { currency: 'USD', showDecimalIfSmallNumberOnly: true })}`}
              domain={([dataMin, dataMax]) => {
                // add 0.1 spacing to min and max data
                const min = Math.floor(dataMin - dataMin * 0.1)
                const max = dataMax > +oraclePrice ? Math.round(dataMax + dataMax * 0.1) : +oraclePrice + 200
                return [min, max]
              }}
            />
            <YAxis
